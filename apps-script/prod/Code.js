...
function handleCreateRequest_(payload, sheet) {
  ...
  return jsonResponse_({
    ...
    message: 'Request received. We will review availability and follow up to confirm.'
  });
}
...
function handleUpdateRequest_(payload, sheet) {
  ...
  return jsonResponse_({
    ...
    status: UPDATED_STATUS,
    message: 'Update received. We will review the changes before confirming.'
  });
}
...
function handleCancelRequest_(payload, sheet) {
  ...
  return jsonResponse_({
    ...
    message: 'Cancellation received. Your request has been marked as cancelled.'
  });
}
...
function sendCustomerRequestReceived_(payload, requestReference) {
  const subject = 'We received your session request';
...
function sendCustomerUpdateReceived_(payload, requestReference) {
  const subject = 'We received your booking request update';
...
function notifyBusinessUpdatedRequest_(payload, requestReference) {
  const subject = `Updated photography booking request: ${requestReference}`;
...
function notifyBusinessCancelledRequest_(values, cancellationNote) {
  const subject = `Cancelled photography booking request: ${values[2]}`;
